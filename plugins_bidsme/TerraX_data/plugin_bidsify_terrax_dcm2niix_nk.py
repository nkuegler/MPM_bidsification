...
def SequenceEP(recording: object) -> int:
    ...
    if recording.Module() == "MRI":
        ...
        match image_type:
            case _ if "DIS3D" in image_type:
                DistCorrType = "3D"
            case _ if "DIS2D" in image_type:
                DistCorrType = "2D"
            case _:
                DistCorrType = None
        ...
        if "ND" in image_type:
            ...
            if not DistCorrType:
                ...
                logger.warning("{}: ImageType 'ND' and DistCorrType '{}' do not match"
                               .format(recording.recIdentity(), DistCorrType))
        ...
        if rec_id.startswith("smap_kp_mtflash3d") or rec_id.startswith("sens_maps_kp_mtflash3d") or \
           rec_id.startswith("smaps_kp_mtflash3d"):
            ...
            ReceiveCoilName = recording.getAttribute("ReceiveCoilName")

            if "head" in ReceiveCoilName.casefold():
            ...
            elif "array" in ReceiveCoilName.casefold():
                ...
                if not head_coil_smap_counter:
                    ...
                    head_coil_smap_counter += 1
                recording.custom["ReceiveCoil"] = current_ReceiveCoil                
            elif "body" in ReceiveCoilName.casefold() or "bc" in ReceiveCoilName.casefold():
...
>>>>>>> 5d6c256 (changed the way TRs are read)
        ...
        if not bidsmap_step:
            ### check that the shim currents are the same for all T1w, PDw, MTw, and (sTx) AFI scans
        ...
        if session_shim_current_relevant_sequences_counter in (0, 1): 
            ...
            if session_shim_current_warning_counter == 0:
                ...
                with open(inconsistency_file, "w") as f:
                    f.write(f"""WARNING: 
                            Shim currents are inconsistent for T1w, PDw, MTw, and (sTx) AFI in {scan.subject} {scan.session}. 
