...
bidsme.prepare(str(SOURCE_PATH), str(PREPARED_PATH),
               ...
               # sub_skip_dir=True,
               sub_list=["sub-020"] # only run on specified subjects (must be specified in BIDS notation)
              )
bidsme.tools.info.reporterrors(logger)
bidsme.tools.info.reseterrors(logger)

PLUGIN_BIDS = WORKING_DIR / "plugins_bidsme" / "plugin_bidsify_auto_nk.py"
