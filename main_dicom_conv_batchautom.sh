...
if [[ $run_dicom_conv = "1" ]]; then
    ...
    if [[ -z ${!last_dcm_path_name} ]] || [[ -z ${!last_nii_path_name} ]]; then
        echo "Error: There must be as many path_to_dcm as path_to_nii specified in the settings file." #>&2
        exit 1
    else
    ...
    for i in $(seq 1 $number_of_paths); do
        ...
        matlab -nodesktop -nosplash -r "call_dicom_conversion('$curr_path_to_dcm','$curr_path_to_nii','$hMRI_dcmConv_excl')"
